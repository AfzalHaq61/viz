...
use Modules\CourseSetting\Entities\FutureCourses;
...
class CourseSettingController extends Controller
{
    ...
    public function saveCourse(Request $request)
    {
        ...
        $rules = [
            ...
            'course_background_image' => 'nullable|mimes:jpeg,bmp,png,jpg|max:1024',
            'course_icon' => 'nullable|mimes:jpeg,bmp,png,jpg|max:1024',
        ];
        ...
        try {
            ...
            if ($request->file('course_background_image') != "") {
                $course->course_background_image = $this->saveImage($request->course_background_image);
            }

            if ($request->file('course_icon') != "") {
                $course->course_icon = $this->saveImage($request->course_icon);
            }

            if ($request->course_feature != "") {
                $course->course_feature = json_encode($request->course_feature);
            }
    ...
    public function getFutureCourse()
    {
        $futureCourses = FutureCourse::all();
        return view('coursesetting::future_courses', compact('futureCourses'));
    }

    public function getFutureCourseCreate()
    {
        return view('coursesetting::create_future_courses');
    }

    public function getFutureCourseStore(Request $request)
    {
        ...
        $image_2 = Image::make($image);

        $image_2->resize(256, 250)->save(public_path('future-courses/' . $filename));

        $created = FutureCourse::create([
            'title' => $request->title,
            'image' => $filename
        ]);

        if ($created) {
            return redirect()->route('getFutureCourse');
        } else {
            return redirect()->back();
        }
    }

    public function getFutureCourseEdit(FutureCourse $futureCourse)
    {

        return view('coursesetting::edit_future_courses', compact('futureCourse'));
    }

    public function getFutureCourseUpdate(Request $request, FutureCourse $futureCourse)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image_2 = Image::make($image);
            $image_2->resize(256, 250)->save(public_path('future-courses/' . $filename));
            $fileNameOuter = $filename;
        }

        $updated = $futureCourse->update([
            'image' => $fileNameOuter ? $fileNameOuter : $prevImage,
            'title' => $request->title,
        ]);
        ...
        if ($updated) {
            ...
            if ($fileNameOuter) {
                if (File::exists(public_path('future-courses/' . $prevImage))) {
                    File::delete(public_path('future-courses/' . $prevImage));
                }
            }
    ...
    public function getFutureCourseDestroy(FutureCourse $futureCourse)
    {

        $img = $futureCourse->image;
        $deleted = $futureCourse->delete();

        if ($deleted) {
            if (File::exists(public_path('future-courses/' . $img))) {
                File::delete(public_path('future-courses/' . $img));
            }
        }
